...
class BlogPagesController extends Controller
{
    ...
    public function index() 
    {
        ...
        $posts = Post::orderBy('created_at', 'desc')->paginate(5);  //Fetches all posts sorted but paginates them and shows 5 posts per page.
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('cover_image')){
            ...
            /*Get just file name. [MN - 11.06.2020]*/
        ...
        //Create Post. [MN - 11.06.2020]
        $post = new Post;
        $post->title = $request->input('title');
        $post->body = $request->input('body');
        /*Adding user_id of the user logged in and creating the post. [MN - 11.06.2020] */
        //$post->user_id = auth()->user()->id;
        $post->user_id = $request->user()->id;
        /*Store uploaded file to posts table in DB. [MN - 11.06.2020]*/
    ...
    public function edit($id)
    {
        /*Fetch posts from DB based upon post id clicked on the posts page rendered through 
            resource==>posts==>blog_index.blade.php. Post::find($id); returns post as an array. [MN - 11.Jun.2020]*/

        $post = Post::find($id);
        /*Check for the legitimate user who can edit a post.
            This provides protection against manually entering the URL to edit other users' post. [MN - 11.Jun.2020]*/
